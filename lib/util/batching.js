...
var remindSendingMessage = function() {
    ...
    if (timeoutRef == null) {
      ...
      timeoutRef = setTimeout(function() {
        ...
        if (currentEventQueue.length > 0) {
          performSendMessage(packEvents(currentEventQueue, configs.version), function(req) {
  ...
  performSendMessage(packEvents(eventQueue, configs.version), function(req) {
    remindSendingMessage();
  });
  stateEventQueue.set([]);
};
...
var checkDehydratedMessage = function() {
  ...
  if (eventQueue.length > 0) {
    performSendMessage(packEvents(eventQueue, configs.version), function() {
      // nothing to to
    });
    stateEventQueue.set([]);
  }
