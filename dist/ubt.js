(function() {
    ...
    // 用于缓存已经发送过的错误信息
    var src$tasks$error$$errorCache = {};

    // 为了防止无限发，这里限制只发送 10 次
    var src$tasks$error$$limit = 10;
    var src$tasks$error$$sendMessage = function(message) {
      if(!message || src$tasks$error$$errorCache[message] || src$tasks$error$$limit <= 0) return;
      src$kernel$$default.send('JSERROR', { message: message });
      src$tasks$error$$errorCache[message] = true;
      src$tasks$error$$limit--;
    };

    if(window.addEventListener) {
      addEventListener('error', function(e) {
        src$tasks$error$$sendMessage(e.error && e.error.stack);
      });
    } else if(window.attachEvent) {
      attachEvent('onerror', function(msg, url, line) {
        src$tasks$error$$sendMessage([msg, 'url: ' + url, 'line: ' + line].join('\r\n'));
      });
    }
    ...
    var src$events$visit$$sended = {};

    var src$events$visit$$visit = function(element) {
      var id = element.getAttribute(src$events$visit$$key);
      if(!src$events$visit$$sended[id]) {
        src$kernel$$default.send('EVENT', {
          id: id,
          params: src$lib$collectdatafrom$$default(element)
        });
        src$events$visit$$sended[id] = true;
      }
      element.removeAttribute(src$events$visit$$key);
    };
    ...
    var src$events$visit$$watch = function() {
      var elements = document.querySelectorAll('[' + src$events$visit$$key + ']');
      for(var i = 0; i < elements.length; i++) src$events$visit$$checkVisibility(elements[i]);
      setTimeout(src$events$visit$$watch, 800);
    };
